...
1. **Validate Parameters** — ensures `GROUP_ID`, `ARTIFACT_ID`, and `VERSION` are provided.
...
| `ARTIFACT_ID` | — | Maven artifact ID (e.g. `mytest`) |
| `VERSION` | — | Release version to promote (e.g. `1.0.0`) |
| `CENTRAL_PUBLISHING_TYPE` | `USER_MANAGED` | `USER_MANAGED`: validate only, manual publish via portal; `AUTOMATIC`: auto-publish |
